  ...
  module ExpandedMounts
    ...
    def self.required_mount_info(char, chargen_data)
      ...
      if !mount_data.join.blank?
        ...
        char_named = Character.named(chargen_data[:custom][:mount_name])
        errors.concat [t('expandedmounts.already_mount_named')] if (mount && mount.bonded != char) || (char_named && char.name != char_named)
